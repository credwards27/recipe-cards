...
class AppManager {
    ...
    */
    constructor() {
        ...
        ipc.handle("get-selected-recipe", this.ipcGetSelectedRecipe);
    }
    ...
    */
    destroy() {
        ...
        ipc.removeHandler("get-selected-recipe");
    }
    ...
    */
    ...
    /* Getter for _data.selected.
        
        Returns the selected recipe ID, or an empty string if no recipe is
        selected.
    */
    get selected() {
    ...
    */
    ipcOpenFile = async (event) => {
        ...
        // Read the file
        try {
            fileData.data = await readFileAsync(filePath, "utf8");
        }
        catch (e) {
        ...
        //
        // TODO: (credwards27) Store full file handle.
        //
    ...
    */
    ...
    /* Gets the currently selected recipe data.
        
        event - IPC event.
    */
    ipcGetSelectedRecipe = (event) => {
        let recipe = this.getRecipe(this.selected);
        return recipe?.data?.serialize();
    };
    ...
    */
    ipcSelectRecipe = async (event, id) => {
        let recipe = this.getRecipe(id),
            selected = this._data.selected;
        
        recipe = recipe?.data?.serialize();
        ...
        this._data.selected = recipe?.id || "";
        
        if (selected === this._data.selected) {
            // Same recipe was already selected, don't emit update event
            return;
        }
