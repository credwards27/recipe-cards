...
const { app, BrowserWindow } = require("electron"),
    ...
    path = require("path"),
    fs = require("fs"),
    copy = require("app/all/deep-copy");
...
const PATH_ROOT = app.getAppPath(),
    ...
    // Custom configuration settings for each window object.
    WINDOW_CONFIGS = {
        output: {
...
/* Gets all existing window instances.
    
    Returns an object containing all existing BrowserWindow instances, keyed by
    window name.
*/
function getAllWindows() {
    ...
    for (let k in WINDOWS) {
        ...
        let instance = WINDOWS?.[k]?.instance;
        
        instance && (windows[k] = instance);
    }
...
function getFullConfig(config, name) {
    ...
    copy(config, fullConfig);
    copy(WINDOW_CONFIGS[name] || {}, fullConfig);
...
HANDLERS["ready-to-show"] = function() {
    ...
    if (this.win.instance === WINDOWS?.output?.instance) {
